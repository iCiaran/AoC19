...
func partA(input string) string {
	...
	x, y := 0, 0
	scaffolding := make(map[coord]int)

	for cpu.running {
		cpu.clock()
		if len(cpu.outputBuffer) > 0 {
			if cpu.outputBuffer[0] == 10 {
				y++
				x = 0
			} else {
	...
	for c := range scaffolding {
		if isCross(scaffolding, c) {
			total += c.x * c.y
		}
	}
...
func partB(input string) string {
	...
	cpu.inputQueue = append(cpu.inputQueue, M...)
